...
- Mounts and environment variables are declared in a **runtime-neutral form**: wrappers and the engine call `add_mount HOST CONTAINER [ro[,nolabel]]`, `add_optional_mount` and `add_env NAME VALUE`, which append to an ordered session spec. A **runtime backend** turns that spec, the image tag and the launch command into the actual command line. The Podman backend (the only one today) adds the rootless flags (`--userns=keep-id`, `:z` labels unless `nolabel`, firewall capabilities) and runs `podman run`. This keeps runtime details in one place and is what allows another backend (e.g. Apptainer inside a Kubernetes pod) to reuse the wrappers unchanged.
