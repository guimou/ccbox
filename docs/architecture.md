...
| **Main config** (the settings/config the harness needs to run; always mounted so the harness behaves correctly) | **Always** | `~/.claude/settings.json`, `settings.local.json`, `~/.claude.json` | whole `~/.config/opencode/` dir (incl. `opencode.json`) | `~/.qwen/settings.json` (+ home `~/.qwen/.env` ro if present) | `~/.codex/config.toml` |
...
| Location | Purpose | Scope |
|----------|---------|-------|
| `~/.codex/config.toml` | Global config (model, providers, settings) | Shared |
